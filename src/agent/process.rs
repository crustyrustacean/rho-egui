...
use super::protocol::{RequestKind, RhoEvent, parse_notification, parse_response_error};
...
impl RhoAgent {
    ...
    /// reader threads. `repaint` is called from reader threads to wake the egui
    /// event loop when new output arrives.
    pub fn spawn(repaint: std::sync::Arc<dyn Fn() + Send + Sync>) -> Result<Self, String> {
        ...
        thread::spawn(move || {
            for line in BufReader::new(stderr).lines() {
                match line {
                    Ok(line) => {
                        if tx.send(RhoOutput::Stderr(line)).is_err() {
                            break;
                        }
                        // stderr doesn't need to trigger repaint; it's debug info
                    }
                    Err(_) => break,
                }
            }
    ...
    pub fn prompt(&mut self, message: &str, steer: bool) -> Result<(), String> {
        self.fire(
            "prompt",
            serde_json::json!({ "message": message, "steer": steer }),
        )
    }
